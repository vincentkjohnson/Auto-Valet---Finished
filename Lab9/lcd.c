...
/**
 * Displays a rotating banner on the LCD screen
 *
...
void lcd_rotatingBanner(char dispText[]){
    ...
    int displayLength = strlen(dispText);
    //Banner Start
    char bannerStart[100];
    strcpy(bannerStart,"                    ");
    bannerStart[20] = '\0';
    ...
    //getting bannerFull ready
    strcat(bannerStart, dispText);
    strcpy(bannerFull, bannerStart);
    strcat(bannerFull, bannerEnd);

    //create a new character array for display
    char display[21];
    ...
    //while loop to move characters through the display
    while (start <= (20 + displayLength)) {
        int i;
        j = 0;

        //Copy 20 character segement from bannerFull string into display string
        for (i = start; i < end; i++) {
            display[j] = bannerFull[i];
            j++;
        }
